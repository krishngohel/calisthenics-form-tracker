...
interface ReadyOverlayProps {
  ...
  bestMs?: number;
  /** Progression goal for this step, e.g. "Hold 30 s". */
  goal?: string;
  ready: boolean;
  onStart: () => void;
}
...
export function ReadyOverlay({ title, guide, cameraAngle, bestMs, goal, ready, onStart }: ReadyOverlayProps) {
  return (
      ...
      <div className="w-full max-w-md rounded-2xl bg-black/70 p-5 text-white backdrop-blur-md">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <span className="text-sm text-white/70">
            {[goal ? `Goal: ${goal.replace(/^Hold /, "hold ")}` : null, bestMs ? `best ${formatMs(bestMs)}` : null].filter(Boolean).join(" · ")}
          </span>
