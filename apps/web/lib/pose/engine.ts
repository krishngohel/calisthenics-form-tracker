...
export type WorkerIn = WorkerInit | WorkerDetect | WorkerBenchmark | { type: "dispose" };
...
/** OffscreenCanvas where available (workers, modern main threads), else a DOM canvas. */
...
function drawToCanvas(bitmap: ImageBitmap): Surface {
  if (!canvas || canvas.width !== bitmap.width || canvas.height !== bitmap.height) {
    canvas = createSurface(bitmap.width, bitmap.height);
 ...
 * read from it.
 */
async function detectBody(source: Surface, timestamp: number): Promise<Body> {
  const body: Body = {};
 ...
 */
function detectHands(source: Surface, timestamp: number, body: Body): Hands {
...
/**
 * Pose inference engine. Runs identically inside a Web Worker (preferred) or
 * on the main thread (fallback for environments where workers cannot be
 * created or never come up, e.g. some custom-scheme WebViews).
 */
export function createPoseEngine(post: (event: EngineEvent) => void) {
  return async function handle(msg: WorkerIn): Promise<void> {
    try {
      if (msg.type === "init") {
        ...
        try {
          if (!initPromise) {
            post({ type: "skipped", timestamp: msg.timestamp });
            return;
          }
